...
2. получает список таблиц из предзагруженного снимка схемы (`ConnectionSchema`, один запрос на подключение);
3. отбрасывает ignored tables;
4. подбирает параметры фильтрации;
5. вызывает `DatabaseService::streamUserData()` (он сам пропустит отсутствующие таблицы);
6. передает stream в `BackupProcessor`.
...
Заголовок секции таблицы пишется лениво — только при первой строке. Таблицы без строк
в файл не попадают: восстанавливать в них нечего, а размер файла и время restore меньше.

### 8.3 Производительность выгрузки

Пакет обходит все таблицы всех подключений, поэтому при большом числе таблиц и/или
удалённой БД доминирует число round-trip × сетевая латентность. Что делает пакет:

- **Предзагрузка схемы** (`ConnectionSchema`): таблицы, колонки, типы и первичные ключи
  читаются одним запросом к `information_schema` на подключение и далее берутся из памяти.
  Это убирает per-table round-trip на `hasTable` / список колонок / определение PK.
- **Keyset-пагинация** (`Builder::lazyById()`) по одиночному числовому первичному ключу
  вместо OFFSET — время выборки не деградирует с глубиной страницы. Для составных или
  нечисловых PK автоматический OFFSET-fallback (`lazy()`).
- **Подзапрос-фильтр** (`FilterSubquery`): вместо `whereIn([десятки тысяч литералов])`
  подставляется `field IN (SELECT col FROM table WHERE col = ?)` — компактный SQL и
  семи-джойн по индексу вместо разбора мегабайтного списка на каждой таблице.
- **chunkSize по умолчанию 5000** — крупные чанки уменьшают число round-trip; keyset
  делает это безопасным независимо от состояния кэша БД.

Остаточное время выгрузки определяется физическим объёмом данных пользователя — его
пагинацией/метаданными убрать нельзя.
...
Таблицы без строк в файл не включаются (в примере выше их просто нет как ключей).
