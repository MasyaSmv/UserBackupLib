...
- `user_subaccounts` сейчас обрабатывается по текущей логике пакета через `accountIds`, если фильтр попал в `id`;
- это предметно чувствительный участок и его нужно держать в поле зрения проекта.
